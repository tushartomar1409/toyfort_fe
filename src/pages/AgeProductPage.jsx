...
const AgeProductPage = () => {
  ...
  const { fetchProductByAge, productByAge } = useContext(AppContext);
  ...
  const currentProduct = productByAge.slice(
    indexOfFirstProduct,
    indexOfLastProduct
  );
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <div className="flex flex-1">
        <Sidebar />
        <div className="p-4">
          {Array.isArray(productByAge) && productByAge.length > 0 ? (
            <div className="w-full ml-5 mt-5">
              <div className="w-full grid grid-cols-4 gap-4">
                {productByAge.map((item, index) => (
                  <Card
      ...
      <div className="flex justify-center mt-5 mb-5">
        <Pagination
          totalPages={Math.ceil(productByAge.length / postsPerPage)}
